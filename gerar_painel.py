...
def calcula_sonda(df: pd.DataFrame) -> list:
    ...
    ATENÇÃO METODOLÓGICA: isso varre título + trecho. Se o trecho for o snippet
    de ~150 caracteres devolvido pela busca do IOERJ, um resultado zero significa
    'não aparece no recorte indexado', NÃO 'ausente do documento'. Só vire a
    contagem em evidência depois de raspar o inteiro teor.
    """
    ...
    registros = [
        {
            ...
            "t": r["titulo_limpo"][:200],
            "x": (r["trecho"] if isinstance(r["trecho"], str) else "")[:200],
            "p": r["poder"],
            "a": r["tipo"],
            "g": r["pagina"],
            "u": r["link"],
